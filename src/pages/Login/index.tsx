...
import { criarSnackbar } from 'src/store/reducers/snackbar';
...
export default function Login({ navigation }: DrawerScreenProps<RootStackParamList, 'Login'>) {
  ...
  const handleLogin = () => {
    if (!emailOuCpf) return dispatch(criarSnackbar({
      tipo: 'error',
      mensagem: 'Preencha um Email ou CPF'
    }));
    if (!senha) return dispatch(criarSnackbar({
      tipo: 'error',
